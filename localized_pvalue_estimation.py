...
class averaged_KLPE_anomaly_detection:
    ...
    def build_knn_graph(self, data):
        ...
        if self.approx_nearest_neighbors:
            ...
            params = {
                ...
                'n_neighbors': max(1 + self.neighborhood_range[1], 20),
                'rho': 0.5,
    ...
    def distance_statistic(self, data, exclude_self=False):
        ...
        if self.approx_nearest_neighbors:
            nn_indices, nn_distances = self.index_knn.query(data, k=k2)
        else:
            nn_distances, nn_indices = self.index_knn.kneighbors(data, n_neighbors=k2)
